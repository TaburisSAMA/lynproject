...
NETSNMP_STATIC_INLINE int _ifTable_check_indexes(ifTable_rowreq_ctx *
                                                 rowreq_ctx);
...
_ifTable_initialize_interface(ifTable_registration * reg_ptr, u_long flags)
{
     ...
     */
    ifTable_container_init_persistence(ifTable_if_ctx.container);
...
NETSNMP_STATIC_INLINE int
_ifTable_check_indexes(ifTable_rowreq_ctx * rowreq_ctx)
{
    int             rc = SNMPERR_SUCCESS;

    DEBUGMSGTL(("internal:ifTable:_ifTable_check_indexes", "called\n"));
    ...
    if ((SNMPERR_SUCCESS == rc)
        && ((rowreq_ctx->tbl_idx.ifIndex < 1)
            || (rowreq_ctx->tbl_idx.ifIndex > 2147483647))
        ) {
        rc = SNMP_ERR_WRONGVALUE;
    }
    if (MFD_SUCCESS != rc)
        return rc;
    rc = ifIndex_check_index(rowreq_ctx);
    if (MFD_SUCCESS != rc)
     ...
     */
    return ifTable_validate_index(ifTable_if_ctx.user_ctx, rowreq_ctx);
}                               /* _ifTable_check_indexes */
...
static int      _ifTable_container_save_rows(int majorID, int minorID,
                                             void *serverarg,
                                             void *clientarg);
static void     _ifTable_container_row_restore(const char *token,
                                               char *buf);
static int      _ifTable_container_row_save(ifTable_rowreq_ctx *
                                            rowreq_ctx, void *type);
static char    *_ifTable_container_col_restore(ifTable_rowreq_ctx *
                                               rowreq_ctx, u_int col,
                                               char *buf);
static char    *_ifTable_container_col_save(ifTable_rowreq_ctx *
                                            rowreq_ctx, u_int col,
                                            char *buf);

static char     row_token[] = "ifTable";
 ...
 */
void
ifTable_container_init_persistence(netsnmp_container * container)
{
    int             rc;

    register_config_handler(NULL, row_token,
                            _ifTable_container_row_restore, NULL, NULL);
    rc = snmp_register_callback(SNMP_CALLBACK_LIBRARY,
                                SNMP_CALLBACK_STORE_DATA,
                                _ifTable_container_save_rows, container);

    if (rc != SNMP_ERR_NOERROR)
        snmp_log(LOG_ERR, "error registering for STORE_DATA callback "
                 "in _ifTable_container_init_persistence\n");
}

static int
_ifTable_container_save_rows(int majorID, int minorID, void *serverarg,
                             void *clientarg)
{
    char            sep[] =
        "##############################################################";
    char            buf[] = "#\n" "# ifTable persistent data\n" "#";
    ...
    CONTAINER_FOR_EACH((netsnmp_container *) clientarg,
                       ...
                       _ifTable_container_row_save, type);
...
/************************************************************
 * _ifTable_container_row_save
 */
static int
_ifTable_container_row_save(ifTable_rowreq_ctx * rowreq_ctx, void *type)
{
    /*
...
#define MAX_ROW_SIZE (sizeof(row_token) + 1 +  \
        ( 12 ) + /* ASN_INTEGER ifAdminStatus */ \
        ( IFTABLE_MAX_COL * 12 ) + /* column num prefix + : */ \
    2 /* LINE_TERM_CHAR + \n */ )
    ...
    if (ifTable_container_should_save(rowreq_ctx) == 0) {
        return SNMP_ERR_NOERROR;
    }
    ...
    if (NULL == pos) {
        snmp_log(LOG_ERR, "error saving ifTable row "
                 "to persistent file\n");
    ...
    *pos++ = ' ';
     ...
     * add each column
     */
    for (i = IFTABLE_MIN_COL; i <= IFTABLE_MAX_COL; ++i) {

        if ((0x1 << (i - 1)) & ~IFTABLE_SETTABLE_COLS)
            continue;

        tmp = pos;
        pos = _ifTable_container_col_save(rowreq_ctx, i, pos);
        if (NULL == pos)
            pos = tmp;
        else
            *pos++ = ' ';
    ...
    DEBUGMSGTL(("internal:ifTable:_ifTable_container_row_save",
                "saving line '%s'\n", buf));
...
static void
_ifTable_container_row_restore(const char *token, char *buf)
{
    ifTable_rowreq_ctx *rowreq_ctx;
    netsnmp_index   index;
    ...
    if (strncmp(token, row_token, sizeof(row_token)) != 0) {
        snmp_log(LOG_ERR,
                 "unknown token in _ifTable_container_row_restore\n");
        return;
    }

    DEBUGMSGTL(("internal:ifTable:_ifTable_container_row_restore",
                "parsing line '%s'\n", buf));
    ...
    if (NULL == buf) {
        snmp_log(LOG_ERR, "error reading row index in "
                 "_ifTable_container_row_restore\n");
        return;
    }
//    rowreq_ctx = _mfd_ifTable_rowreq_from_index(&index, NULL);
    if (NULL == rowreq_ctx) {
    ...
    while ((NULL != buf) && isdigit(*buf)) {
         ...
         * parse value
         */
        DEBUGMSGTL(("_ifTable_container_row_restore",
                    "parsing column %d\n", col));
        buf = _ifTable_container_col_restore(rowreq_ctx, col, buf);
        ++found;
    }
    if (0 == found) {
        snmp_log(LOG_ERR, "error parsing ifTable row; no columns found\n");
        ifTable_release_rowreq_ctx(rowreq_ctx);
        return;
    }
    ...
    if ((buf == NULL) || (*buf != LINE_TERM_CHAR)) {
        snmp_log(LOG_ERR, "error parsing ifTable row around column %d\n",
                 col);
        ifTable_release_rowreq_ctx(rowreq_ctx);
        return;
    }
    ...
    CONTAINER_INSERT(ifTable_if_ctx.container, rowreq_ctx);
}

/************************************************************
 * _ifTable_container_col_save
 */
static char    *
_ifTable_container_col_save(ifTable_rowreq_ctx * rowreq_ctx,
                            u_int col, char *buf)
{
    ...
    DEBUGMSGTL(("internal:ifTable:_ifTable_container_col_save",
                "processing column %d\n", col));
    ...
    case COLUMN_IFDESCR:    /** DisplayString = ASN_OCTET_STR */
        buf = read_config_save_octet_string(buf, rowreq_ctx->data.ifDescr,
                                            rowreq_ctx->data.ifDescr_len);
        break;

    case COLUMN_IFTYPE:    /** IANAifType = ASN_INTEGER */
        buf += sprintf(buf, "%ld", rowreq_ctx->data.ifType);
        break;

    case COLUMN_IFMTU:    /** INTEGER32 = ASN_INTEGER */
        buf += sprintf(buf, "%ld", rowreq_ctx->data.ifMtu);
        break;

    case COLUMN_IFSPEED:    /** GAUGE = ASN_GAUGE */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifSpeed);
        break;

    case COLUMN_IFPHYSADDRESS:    /** PhysAddress = ASN_OCTET_STR */
        buf =
            read_config_save_octet_string(buf,
                                          rowreq_ctx->data.ifPhysAddress,
                                          rowreq_ctx->data.
                                          ifPhysAddress_len);
        break;

    case COLUMN_IFADMINSTATUS:    /** INTEGER = ASN_INTEGER */
        buf += sprintf(buf, "%ld", rowreq_ctx->data.ifAdminStatus);
        break;

    case COLUMN_IFOPERSTATUS:    /** INTEGER = ASN_INTEGER */
        buf += sprintf(buf, "%ld", rowreq_ctx->data.ifOperStatus);
        break;

    case COLUMN_IFLASTCHANGE:    /** TICKS = ASN_TIMETICKS */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifLastChange);
        break;

    case COLUMN_IFINOCTETS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifInOctets);
        break;

    case COLUMN_IFINUCASTPKTS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifInUcastPkts);
        break;

    case COLUMN_IFINNUCASTPKTS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifInNUcastPkts);
        break;

    case COLUMN_IFINDISCARDS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifInDiscards);
        break;

    case COLUMN_IFINERRORS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifInErrors);
        break;

    case COLUMN_IFINUNKNOWNPROTOS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifInUnknownProtos);
        break;

    case COLUMN_IFOUTOCTETS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifOutOctets);
        break;

    case COLUMN_IFOUTUCASTPKTS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifOutUcastPkts);
        break;

    case COLUMN_IFOUTNUCASTPKTS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifOutNUcastPkts);
        break;

    case COLUMN_IFOUTDISCARDS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifOutDiscards);
        break;

    case COLUMN_IFOUTERRORS:    /** COUNTER = ASN_COUNTER */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifOutErrors);
        break;

    case COLUMN_IFOUTQLEN:    /** GAUGE = ASN_GAUGE */
        buf += sprintf(buf, "%lu", rowreq_ctx->data.ifOutQLen);
        break;

    case COLUMN_IFSPECIFIC:    /** OBJECTID = ASN_OBJECT_ID */
        buf = read_config_save_objid(buf, rowreq_ctx->data.ifSpecific,
...
/************************************************************
 * _ifTable_container_col_restore
 */
static char    *
_ifTable_container_col_restore(ifTable_rowreq_ctx * rowreq_ctx,
                               u_int col, char *buf)
{
    size_t          len;
    if ((NULL == rowreq_ctx) || (NULL == buf)) {
    ...
    DEBUGMSGTL(("verbose:ifTable:_ifTable_container_col_restore",
                "processing column %d\n", col));
    ...
    case COLUMN_IFDESCR:    /** DisplayString = ASN_OCTET_STR */
        ...
        buf = read_config_read_memory(ASN_OCTET_STR, buf,
                                      (char *) &rowreq_ctx->data.ifDescr,
                                      (size_t *) &rowreq_ctx->data.
                                      ifDescr_len);
        break;

    case COLUMN_IFTYPE:    /** IANAifType = ASN_INTEGER */
        len = sizeof(rowreq_ctx->data.ifType);
        buf = read_config_read_memory(ASN_INTEGER, buf,
                                      (char *) &rowreq_ctx->data.ifType,
                                      &len);
        break;

    case COLUMN_IFMTU:    /** INTEGER32 = ASN_INTEGER */
        len = sizeof(rowreq_ctx->data.ifMtu);
        buf = read_config_read_memory(ASN_INTEGER, buf,
                                      (char *) &rowreq_ctx->data.ifMtu,
                                      &len);
        break;

    case COLUMN_IFSPEED:    /** GAUGE = ASN_GAUGE */
        len = sizeof(rowreq_ctx->data.ifSpeed);
        buf = read_config_read_memory(ASN_GAUGE, buf,
                                      (char *) &rowreq_ctx->data.ifSpeed,
                                      &len);
        break;

    case COLUMN_IFPHYSADDRESS:    /** PhysAddress = ASN_OCTET_STR */
        rowreq_ctx->data.ifPhysAddress_len =
            sizeof(rowreq_ctx->data.ifPhysAddress);
        buf =
    ...
    case COLUMN_IFADMINSTATUS:    /** INTEGER = ASN_INTEGER */
        len = sizeof(rowreq_ctx->data.ifAdminStatus);
        buf = read_config_read_memory(ASN_INTEGER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifAdminStatus, &len);
        break;
    ...
    case COLUMN_IFOPERSTATUS:    /** INTEGER = ASN_INTEGER */
        len = sizeof(rowreq_ctx->data.ifOperStatus);
        buf = read_config_read_memory(ASN_INTEGER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifOperStatus, &len);
        break;
    ...
    case COLUMN_IFLASTCHANGE:    /** TICKS = ASN_TIMETICKS */
        len = sizeof(rowreq_ctx->data.ifLastChange);
    ...
    case COLUMN_IFINOCTETS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifInOctets);
    ...
    case COLUMN_IFINUCASTPKTS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifInUcastPkts);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifInUcastPkts, &len);
        break;
    ...
    case COLUMN_IFINNUCASTPKTS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifInNUcastPkts);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifInNUcastPkts, &len);
        break;
    ...
    case COLUMN_IFINDISCARDS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifInDiscards);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifInDiscards, &len);
        break;
    ...
    case COLUMN_IFINERRORS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifInErrors);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifInErrors, &len);
        break;
    ...
    case COLUMN_IFINUNKNOWNPROTOS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifInUnknownProtos);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifInUnknownProtos, &len);
        break;

    case COLUMN_IFOUTOCTETS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifOutOctets);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifOutOctets, &len);
        break;
    ...
    case COLUMN_IFOUTUCASTPKTS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifOutUcastPkts);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifOutUcastPkts, &len);
        break;
    ...
    case COLUMN_IFOUTNUCASTPKTS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifOutNUcastPkts);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifOutNUcastPkts, &len);
        break;
    ...
    case COLUMN_IFOUTDISCARDS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifOutDiscards);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifOutDiscards, &len);
        break;
    ...
    case COLUMN_IFOUTERRORS:    /** COUNTER = ASN_COUNTER */
        len = sizeof(rowreq_ctx->data.ifOutErrors);
        buf = read_config_read_memory(ASN_COUNTER, buf,
                                      (char *) &rowreq_ctx->data.
                                      ifOutErrors, &len);
        break;
    ...
    case COLUMN_IFOUTQLEN:    /** GAUGE = ASN_GAUGE */
        len = sizeof(rowreq_ctx->data.ifOutQLen);
        buf = read_config_read_memory(ASN_GAUGE, buf,
                                      (char *) &rowreq_ctx->data.ifOutQLen,
                                      &len);
        break;

    case COLUMN_IFSPECIFIC:    /** OBJECTID = ASN_OBJECT_ID */
        rowreq_ctx->data.ifSpecific_len =
            sizeof(rowreq_ctx->data.ifSpecific);
        buf =
            read_config_read_memory(ASN_OBJECT_ID, buf,
                                    (char *) &rowreq_ctx->data.ifSpecific,
                                    (size_t *) &rowreq_ctx->data.
                                    ifSpecific_len);
        rowreq_ctx->data.ifSpecific_len /= sizeof(oid);
        break;
