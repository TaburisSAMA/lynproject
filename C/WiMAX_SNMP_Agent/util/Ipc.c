...
{
	if (vBFp_wait == IPC_FALSE)
		return msgrcv(vS32Fp_msgQID, pvVFp_msgBuffer, (size_t) PACKET_SIZE, vLFp_msgType, IPC_NOWAIT);

	else
		return msgrcv(vS32Fp_msgQID, pvVFp_msgBuffer, (size_t) PACKET_SIZE, vLFp_msgType, 0);
}
